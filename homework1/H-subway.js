...
const getMinTime = (trainCount, intervalTime) => trainCount + intervalTime * (trainCount - 1);
const getMaxTime = (trainCount, intervalTime) => trainCount + intervalTime * (trainCount + 1);

const func = (a, b, n, m) => {
  const minTimePlatformA = getMinTime(n, a);
  const maxTimePlatformA = getMaxTime(n, a);
  // console.log('a', minTimePlatformA, maxTimePlatformA)

  const minTimePlatformB = getMinTime(m, b);
  const maxTimePlatformB = getMaxTime(m, b);
  // console.log('b', minTimePlatformB, maxTimePlatformB)

  const min = Math.max(minTimePlatformA, minTimePlatformB);
  const max = Math.min(maxTimePlatformA, maxTimePlatformB);
...
/*
  ...
  На некоторых кросс-платформенных станциях метро (как, например, «Третьяковская») на 
  разные стороны платформы приходят поезда разных направлений. Таня договорилась встретиться с подругой на такой станции,
