...
const withstandPressure = strings[1].split(' ').map(Number);
const k = Number(strings[2]);
const seqOfKeyPressure = strings[3].split(' ').map(Number);
...
const result = [];
for (let i = 0; i < n; i += 1) {
  withstandPressure[i] < 0 ? result.push("YES") : result.push("NO");
}
...
то для определенной последовательности нажатых клавиш можно определить,какие клавиши в
