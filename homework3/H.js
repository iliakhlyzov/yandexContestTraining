...
const birdsCount = Number(strings[0]);
const birdPoints = [];
for (let i = 1; i <= birdsCount; i += 1) {
  birdPoints.push(strings[i].split(' ').map(Number));
}

const slow = (birdPoints) => {
  let birdsAboveStartingPoint = 0;
  const highestBirdsPoint = {};
  birdPoints.forEach(([x, y]) => {
    if (x === 0) {
      birdsAboveStartingPoint += 1;
      return;
    }
    highestBirdsPoint[x] = highestBirdsPoint.x 
      ? Math.max(highestBirdsPoint[x], y) : y;
  });
  return Object.keys(highestBirdsPoint).length + birdsAboveStartingPoint;
}
console.log(slow(birdPoints));
